...
#define H_API_VSNWPRINTF		0x6e61dbc5 /* _vsnwprintf */
...
D_SEC( B ) PBUFFER BufferCreate( VOID )
{
	...
	Api._vsnwprintf       = PeGetFuncEat( PebGetModule( H_LIB_NTDLL ), H_API_VSNWPRINTF );
...
D_SEC( B ) BOOL BufferPrintfW( _In_ PBUFFER Buffer, _In_ PWSTR Format, ... )
{
	...
	/* Get length of buffer */
	va_start( Lst, Format );
	Len = Api._vsnwprintf( NULL, 0, Format, Lst );
	va_end( Lst );
	...
	if ( Buf.Buffer != NULL ) {
		...
		/* Copy over buffer */
		va_start( Lst, Format );
		Len = Api._vsnwprintf( C_PTR( U_PTR( Buffer->Buffer ) + Buffer->Length ), Len, Format, Lst );
		va_end( Lst );
