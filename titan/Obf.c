 ...
 *
!*/
static DECLSPEC_NOINLINE D_SEC( E ) VOID WINAPI ThreadCall( _In_ PF_PARAM Fiber )
{
...
static DECLSPEC_NOINLINE D_SEC( E ) VOID WINAPI CfgAddAddr( _In_ PVOID ImageBase, _In_ PVOID Function )
{
	API			Api;
...
static D_SEC( E ) VOID WINAPI ObfSystemCallFiber( _In_ PF_PARAM Fbr )
{
	...
	if ( Adv != NULL ) {
		...
		Api.RtlInitAnsiString( &Ani, C_PTR( G_SYM( "SystemFunction036" ) ) );
		Api.LdrGetProcedureAddress( Adv, &Ani, 0, &Api.SystemFunction036 );

		Api.SystemFunction036( &Rnd, 0x10 );
